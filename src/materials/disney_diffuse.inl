Spectrum eval_op::operator()(const DisneyDiffuse &bsdf) const {
    ...
    Vector3 half = (dir_in + dir_out) / length(dir_in + dir_out);
    Real roughness = eval(bsdf.roughness, vertex.uv, vertex.uv_screen_size, texture_pool);
    double n_dot_in = fmax(dot(frame.n, dir_in), Real(0));
    double n_dot_out = fmax(dot(frame.n, dir_out), Real(0));
    double half_dot_out = fmax(dot(half, dir_out), Real(0));
