...
#include "debug.h"
// declare the final tracer first
...
Spectrum get_sigma_t(const Medium& medium, const Vector3& point) {
    Spectrum sigma_a = get_sigma_a(medium, point);
    Spectrum sigma_s = get_sigma_s(medium, point);
...
Spectrum next_event_estimation(const Scene& scene, Vector3 p, const Ray& ray, const int current_medium_id, 
                            pcg32_state& rng, int& bounces, RayDifferential& raydiff, const int& x, const int& y, const PathVertex& pv, bool isScatter) {
    Vector2 light_uv{next_pcg32_real<Real>(rng), next_pcg32_real<Real>(rng)};
    Real light_w = next_pcg32_real<Real>(rng);
    Real shape_w = next_pcg32_real<Real>(rng);
    int light_id = sample_light(scene, light_w);
    ...
    while(true) {
        ...
        if(shadow_medium_id != -1) {
            ...
            Spectrum sigma_t = get_sigma_t(shadow_medium, make_zero_spectrum());
    ...
    if (T_light.x > 0) { // still homogeneous
        //TODO: add bsdf (check the origin point type)
        ...
        //*=============================
        //* for phase function (scatter)
        //*=============================
        if(isScatter && current_medium_id != -1) {
            // assert(current_medium_id != -1);
            ...
            if(debug(x, y)) {
                std::cout << "phase contrib " << w * contrib << std::endl;
                std::cout << "G " << G << std::endl;
                std::cout << "f " << f << std::endl;
                std::cout << "L " << L << std::endl;
                std::cout << "pdf_nee " << pdf_nee << std::endl;
                std::cout << "pdf_phase " << pdf_phase << std::endl;
        ...
        const Material& mat = scene.materials[pv.material_id];
        Spectrum f = eval(mat, dir_view, dir_light, pv, scene.texture_pool);
        //?need light_pmf?
        Real pdf_nee = light_pmf(scene, light_id) * pdf_point_on_light(light, p_prime, ori_p, scene);
        Spectrum contrib = T_light * G * f * L / pdf_nee;
        ...
        Spectrum pdf_bsdf = pdf_sample_bsdf(
                    mat, dir_view, dir_light, pv, scene.texture_pool) * G * p_trans_dir;
        Spectrum w = (pdf_nee * pdf_nee) / (pdf_nee * pdf_nee + pdf_bsdf * pdf_bsdf);
        // Spectrum w = make_const_spectrum(0);
        if(debug(x, y)) {
            std::cout << "bsdf contrib " << w * contrib << std::endl;
            std::cout << "G " << G << std::endl;
            std::cout << "f " << f << std::endl;
            std::cout << "L " << L << std::endl;
            std::cout << "pdf_nee " << pdf_nee << std::endl;
            std::cout << "pdf_bsdf " << pdf_bsdf << std::endl;
            std::cout << "w " << w << std::endl;
        }
        return w * contrib;
    }         
...
Spectrum vol_path_tracing_4(const Scene &scene,
    ...
    //         nee_p_cache = p; 
    //         Spectrum nee = next_event_estimation(scene, p, ray, current_medium_id, rng, bounces, ray_diff, x, y, *vertex_, true);
    ...
    // // }
    // return radiance;
    vol_path_tracing_5(scene, x, y, rng);
}
...
Spectrum vol_path_tracing_5(const Scene &scene,
    ...
    while (true) {
        ...
        Real t_hit = infinity<Real>();
        if(debug(x, y)) {
            std::cout << "========= ray =============" << ray.org << ' ' << ray.dir << ' ' << ray.tfar << ' ' << ray.tnear << std::endl;       
        }
        std::optional<PathVertex> vertex_ = intersect(scene, ray, ray_diff);
        if(vertex_) {
            if(debug(x, y))
                std::cout << "========= ray hit something ============= " << (*vertex_).shape_id << std::endl;
            t_hit = distance((*vertex_).position, ray.org);
        }
        Spectrum transmittance = make_const_spectrum(1);
        Spectrum trans_pdf = make_const_spectrum(1);
        ...
        if (current_medium_id != -1) {
            ...
            if(t.x < t_hit) {
                ...
                if(debug(x,y)) {
                    std::cout << "======== hit surface "<< (*vertex_).shape_id << "=========" << std::endl;
                    std::cout << "========= t =============" << t << std::endl;
                    std::cout << "========= t_hit =============" << t_hit << std::endl;
                }
            }
        ...
        //TODO update the ray org here will be better
        auto origin_current_path_throughput = current_path_throughput;
        current_path_throughput *= (transmittance / trans_pdf);
        if(debug(x,y)) {
            std::cout << "current_path_throughput " << current_path_throughput << std::endl;
        ...
        if (!scatter && vertex_) {
            ...
            if(is_light(scene.shapes[(*vertex_).shape_id])) {
                if (never_scatter) {
                    ...
                    //TODO: account for multiple sampling
                    //TODO: BSDF (seems that no change needed)
                    // account for the next event estimation
                    PointAndNormal light_point;
                    light_point.position = (*vertex_).position;
                    light_point.normal = (*vertex_).geometric_normal;
        ...
        if (scatter && (current_medium_id != -1)) {
            ...
            //hit a surface (!scatter and vertex is not a light)
            //TODO: add BSDF
            //*=========================================================
            //* first do the next estimation estimation
            //*=========================================================
            ...
            nee_p_cache = p;
            never_scatter = false;
            Spectrum nee = next_event_estimation(scene, p, ray, current_medium_id, rng, bounces, ray_diff, x, y, (*vertex_), false);
            radiance += current_path_throughput * nee;
            if(debug(x, y)) { 
                std::cout << "nee for bsdf" << nee << std::endl;
                std::cout << "curr path through" << current_path_throughput << std::endl;
                std::cout << "radiance in nee for bsdf" << radiance << std::endl;
            }

            //*=========================================================
            //* then do the hemispherical sampling
            //*=========================================================
            Vector3 dir_view = -ray.dir;
            ...
            ray = Ray{(*vertex_).position, dir_bsdf, get_intersection_epsilon(scene), infinity<Real>()};
            multi_trans_pdf = make_const_spectrum(1);
        }
...
Spectrum next_event_estimation_with_ratio_tracking(const Scene& scene, Vector3 p, const Ray& ray, const int current_medium_id, 
                            pcg32_state& rng, int& bounces, RayDifferential& raydiff, const int& x, const int& y, const PathVertex& pv, bool isScatter) {
    Vector2 light_uv{next_pcg32_real<Real>(rng), next_pcg32_real<Real>(rng)};
    Real light_w = next_pcg32_real<Real>(rng);
    Real shape_w = next_pcg32_real<Real>(rng);
    ...
    while(true) {
        ...
        // }
        if(debug(x, y)) {
                std::cout << "shadow_medium_id" << shadow_medium_id << std::endl;
        }
        if(shadow_medium_id != -1) {
            // account for transmittance to next_t
            // in medium
            Real v = next_pcg32_real<Real>(rng);
            int channel = std::clamp(int(v*3), 0, 2);
    
            int iteration = 0;
            // evaluate the transmittance only for this next_t
            Real accum_t = 0;
            while(true) {
                const Medium& shadow_medium = scene.media[shadow_medium_id];
                //! notice: it's shadow_ray (not ray)
                Spectrum sigma_m = get_majorant(shadow_medium, shadow_ray);
                ...
                //sample distance
                Real u = next_pcg32_real<Real>(rng);
                Real t = - log(1 - u) / sigma_m[channel];
                Real dt = next_t - accum_t;
                accum_t = min(accum_t + t, next_t);
                Vector3 current_p = shadow_ray.org + accum_t * shadow_ray.dir;
                Spectrum sigma_t = get_sigma_t(shadow_medium, current_p);
                if(t < dt) {
                    // haven't reach the surface
                    // null-scattering event
                    Spectrum sigma_n = sigma_m * (1 - sigma_t / sigma_m);
                    T_light *= exp(-sigma_m * t) * sigma_n / max(sigma_m);
                    ...
                    if(debug(x, y)) {
                        ...
                        std::cout << "haven't reach the surface" << std::endl;
                        std::cout << "t " << t << std::endl;
                        std::cout << "dt " << dt << std::endl;
                        std::cout << "accum_t " << accum_t << std::endl;
                        std::cout << "next_t " << next_t << std::endl;
                        std::cout << "sigma_t " << sigma_t << std::endl;
                        ...
                        std::cout << "T_light " << T_light << std::endl;
                        std::cout << "p_trans_dir" << p_trans_dir << std::endl;
                        std::cout << "p_trans_nee" << p_trans_nee << std::endl;
                    }
                    ...
                    if(debug(x, y)) {
                        ...
                        std::cout << shadow_bounces << " reach the surface" << std::endl;
                        std::cout << "ori_t " << ori_T << std::endl;
                        std::cout << "next_t " << next_t << std::endl;
                        std::cout << "sigma_t " << sigma_t << std::endl;
                        std::cout << "sigma_m " << sigma_m << std::endl;
                        std::cout << "T_light " << T_light << std::endl;
                        std::cout << "exp " << exp(-sigma_m * next_t) << std::endl;
                        std::cout << "p_trans_dir" << p_trans_dir << std::endl;
                        std::cout << "p_trans_nee" << p_trans_nee << std::endl;
                    }
                    iteration++;
        ...
        if(!isect) {
            ...
            p = p + next_t * shadow_ray.dir;
            if(debug(x, y)) {
                std::cout << "prev p " << prev_p << std::endl;

                std::cout << "p " << p << std::endl;
                // std::cout << "sigma_t " << sigma_t << std::endl;
                std::cout << "next_t " << next_t << std::endl;
                std::cout << "shadow_ray.dir " << shadow_ray.dir << std::endl;
            }
        }
    }
    //?? need check current_medium_id
    if (max(T_light) > 0) { 
        ...
        if(isScatter && current_medium_id != -1) {
            ...
            PhaseFunction phase_function = get_phase_function(scene.media[current_medium_id]);
            Spectrum f = eval(phase_function, -ray.dir, dir_light);
            Spectrum pdf_nee = p_trans_nee * light_pmf(scene, light_id) * pdf_point_on_light(light, p_prime, ori_p, scene);
            //? avg RGB
            Spectrum contrib = T_light * G * f * L / average(pdf_nee);
...
Spectrum vol_path_tracing(const Scene &scene,
    ...
    // if (y > last_y) {
    //     last_y = y;
    //     std::cout << "x: " << last_x << " y: " << last_y << std::endl;
    //     last_x = -1;
    // }
    if(debug(x, y))
        std::cout << "========= new round =============" << std::endl;
    ...
    while (true) {
        ...
        if(vertex_) {
            if(debug(x, y))
                std::cout << "========= ray hit something: " << (*vertex_).shape_id  << ' ' << " is light " << is_light(scene.shapes[(*vertex_).shape_id]) << std::endl;
            t_hit = distance((*vertex_).position, ray.org);
        }
        Spectrum transmittance = make_const_spectrum(1);
        // PDF for free-flight sampling
        Spectrum trans_dir_pdf = make_const_spectrum(1);
        // PDF for next event estimation
        ...
        if (current_medium_id != -1) {
            if(debug(x,y)) {
                std::cout << "========= into medium =============" << std::endl;
            }
            const Medium& current_medium = scene.media[current_medium_id];
            Spectrum sigma_m = get_majorant(current_medium, ray);
            ...
            while(true) {
                ...
                if(t < dt) {
                    Vector3 current_p = ray.org + accum_t * ray.dir;
                    Spectrum sigma_t = get_sigma_t(current_medium, current_p);
                    // sample from real/fake particle events
                    Spectrum real_prob = sigma_t / sigma_m;
                    Real r = next_pcg32_real<Real>(rng);
                    if(r < real_prob[channel]) {
                        // hit a real particle
                        if(debug(x,y)) {
                            std::cout << "========= hit real particle =============" << std::endl;
                        }
                        scatter = true;
                        never_scatter = false;
                        // change "=" to "*=", at the beginning of each loop, reset those value to 1
                        // only account for transmittance in this loop (t_hit)
                        trans_dir_pdf *= exp(-sigma_m * t) * sigma_m * real_prob / max(sigma_m);
                        ...
                        // hit a fake particle
                        if(debug(x,y)) {
                            std::cout << "========= hit fake particle =============" << std::endl;
                        }
                        trans_dir_pdf *= exp(-sigma_m * t) * sigma_m * (1 - real_prob) / max(sigma_m);
                        Spectrum sigma_n = sigma_m * (1 - sigma_t / sigma_m);
                        transmittance *= exp(-sigma_m * t) * sigma_n / max(sigma_m);
                        trans_nee_pdf *= exp(-sigma_m * t) * sigma_m / max(sigma_m);
                    ...
                    trans_dir_pdf *= exp(-sigma_m * dt);
                    transmittance *= exp(-sigma_m * dt);
                    trans_nee_pdf *= exp(-sigma_m * dt);
                    ray.org = ray.org + t_hit * ray.dir;
                    //!! avoid self intersection
                    ray = Ray{ray.org, ray.dir, get_intersection_epsilon(scene), infinity<Real>()};
                    break;
        ...
        //     std::cout << "original current_path_throughput " << origin_current_path_throughput << std::endl;
        //     std::cout << "transmittance " << transmittance << std::endl;
        //     std::cout << "trans_pdf " << trans_dir_pdf << std::endl;
        // }
        ...
        if (!scatter && vertex_) {
            ...
            if(is_light(scene.shapes[(*vertex_).shape_id])) {
                if (never_scatter) {
                    ...
                    // Spectrum pdf_nee = multi_nee_pdf * pdf_point_on_light(light, light_point, nee_p_cache, scene) ;
        ...
        if (scatter && (current_medium_id != -1)) {
            // hit particle
            const Medium& current_medium = scene.media[current_medium_id];
            Vector3 current_p = ray.org;
            Spectrum sigma_s = get_sigma_s(current_medium, current_p);
            const PhaseFunction& phase_function = get_phase_function(current_medium);
            //*=========================================================
            //* first do the next estimation estimation
            //*=========================================================
            //??? multiple the rsult with the transmittance and sigma_s
            Vector3 p = ray.org; // current position
            nee_p_cache = p; 
            Spectrum nee = next_event_estimation_with_ratio_tracking(scene, p, ray, current_medium_id, rng, bounces, ray_diff, x, y, (*vertex_), true);
            ...
            // assert(max(radiance) < 100000);
            // assert(radiance.x == radiance.x && radiance.y == radiance.y && radiance.z == radiance.z);
            ...
            nee_p_cache = p;
            never_scatter = false;
            Spectrum nee = next_event_estimation_with_ratio_tracking(scene, p, ray, current_medium_id, rng, bounces, ray_diff, x, y, (*vertex_), false);
