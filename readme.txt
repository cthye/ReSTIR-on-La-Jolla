...
- For smooth material (roughness is small), the edge is darken. For rough material, the edge is lighter.
- The f_diffuse at grazing angle is attenuated by the lower roughness.
...
- The Oren-Nayar fails to get a fall at grazing retro-reflection for smooth materials compared with DisneyBSDF.
...
## Metal 
1. Compare DisneyMetal with the rough plastic material (try to render images with both BRDFs). What
differences do you see? Why?
- DisneyMetal is anisotropic, having a stronger concentration and longer tail highlight.
- DisneyMetal has a NDF with longer tails as well as considering anisotropic parameter (an ellipsoid distribution of normals).
...
3. A popular alternative over the Trowbridge-Reitz normal distribution function is the Beckmann distribution (a Gaussian distribution on the slopes of the normals). What are the differences
...
- The difference lies on the tail and peak: GGX has a longer tail and falls more slowly than Beckmann.
...
- Cons: not physically accurate in spectral spectrum; inaccurate for eta closed 1
...
- Cons: more computation; shorter tailed highlight;
- I'd prefer the modified normal distribution function since is has a longer tail and wider highlight, which is why we need a clearcoat lobe.
...
- Also, being isotropic simplifies the computation.
...
- The reflection is stronger when η is larger
- The outer layer looks transparent when η is near to 1.
...
- The light transmitted into object will escape from the open boundary (and darken the object?). Using thin-surface BSDF can simulate both the entrance and exit scattering events at a single shading point and get an appearance for a refractive object as if it's closed.
...
- for eta = 1.01, the Schlick approximation's result is much brighter than the other's.
...
appearance?
- Specular describes how the dielectric layer (the one upon the diffuse layer in diffuse lobe) reflects while metallic describes how it looks like a metal (at least their meaning are different)
